...
- **TensorRT optional**: Linux setup can build the TensorRT plugin (`core.trt`) only when CUDA Toolkit plus TensorRT SDK development files are present. If they are missing, setup continues with ONNX Runtime backends.
