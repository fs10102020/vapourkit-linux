...
export function useBatchConfig(options: UseBatchConfigOptions) {
  const { ffmpegArgs, processingFormat, outputFormat, videoCompareArgs, selectedModel, filters, backend, numStreams, segment, colorimetry, showQueue, descriptiveNamingEnabled, onAddToQueue, onLoadVideoInfo, onLog } = options;
